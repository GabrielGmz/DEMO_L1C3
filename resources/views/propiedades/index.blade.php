        ...
        @if(session('user'))
            ...
            @elseif(session('user')->rol == 'propietario' && session('user')->id == $p->id_user)
                <form action="{{ route('propiedades.destroy', $p->id) }}" method="POST" style="display:inline-block;">
                    @csrf
                    @method('DELETE')
                    <button type="submit" onclick="return confirm('¿Estás seguro de eliminar esta propiedad?')" style="background-color: #dc3545; color: white; padding: 8px 12px; border: none; border-radius: 5px;">Eliminar</button>
                </form>
            @endif
        @endif
    </div>
